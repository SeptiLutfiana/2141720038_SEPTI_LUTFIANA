            ...
            @foreach ($LG as $item)
                <tr>
                    <td class="text-center" style="width: 50px;">
                        {{ $loop->iteration + ($LG->currentPage() - 1) * $LG->perPage() }}</td>
                    <td>{{ $item->nama_LG }}</td>
                        ...
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu dropdown-menu-right p-1" style="min-width: 130px; width: 130px;">
                                <a class="dropdown-item d-flex align-items-center py-1"
                                    href="{{ route('adminsdm.data-master.karyawan.learning-group.edit', $item->id_LG) }}">
                                    <i class="fas fa-edit text-warning mr-2" style="width: 18px;"></i> Edit
                                </a>
                                <a class="dropdown-item d-flex align-items-center py-1"
                                    href="{{ route('adminsdm.data-master.karyawan.learning-group.show', $item->id_LG) }}">
                                ...
                                <a href="#" class="dropdown-item text-danger d-flex align-items-center py-1"
                                    onclick="event.preventDefault(); if(confirm('Yakin ingin menghapus data ini?')) document.getElementById('delete-lg-{{ $item->id_LG }}').submit();">
                                    <i class="fas fa-trash-alt mr-2" style="width: 18px;"></i> Hapus
                                </a>
                                <form id="delete-lg-{{ $item->id_LG }}"
                                    action="{{ route('adminsdm.data-master.karyawan.learning-group.destroy', $item->id_LG) }}"
